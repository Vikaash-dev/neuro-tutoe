...
| Implicit contradiction repair | Learners often self-correct when a new claim conflicts with stronger prior knowledge. The tutor should make that internal comparison visible before the wrong fact is rehearsed. | Detect reversed comparison claims such as "sound is faster than light" against trusted evidence like "light is faster than sound," then require the learner to rewrite the relation and name the cue. | Implemented baseline for faster/slower comparison conflicts; broader semantic contradiction detection is needed. |
...
| UX for learning | Interface should encourage attempts, not passive reading. | Attempt panel, confidence slider, hint steps, teach-back, review queue, and optional recall-friction typography such as Sans Forgetica when available. | Partial: recall textareas include a Sans Forgetica font hook with accessible fallbacks. |
