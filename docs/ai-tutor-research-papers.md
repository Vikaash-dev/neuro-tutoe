...
For the expanded Feynman and learning-science source map, see
`docs/learning-science-source-map.md`. That document separates Feynman as a
learning-design contract from the cognitive-science and AI-tutoring paper
spine.

## Feynman Contract Summary

The app should use Feynman as a behavioral test for understanding, not as a
branding label. A learner has not mastered a concept until they can explain it
plainly, name the mechanism, give an example and non-example, identify the
common trap, transfer it to a new case, and say what evidence would prove their
explanation wrong.
...
|---|---|---|---|---|
| Feynman Lectures on Physics, Caltech online edition, `https://feynmanlectures.caltech.edu/`; Feynman, "Cargo Cult Science" (1974), `https://calteches.library.caltech.edu/51/2/CargoCult.htm` | Feynman-style learning means mechanisms over labels, plain explanation over jargon, and intellectual honesty about what would disprove the idea. | Treat teach-back as a serious understanding test: mechanism, example, trap, uncertainty, and transfer. | `src/tutor-engine.js`, `public/app.js`, `tests/tutor-engine.test.js` | Add stronger claim extraction, counterexample prompts, and learner-built derivation tasks. |
...
| Roediger and Karpicke, "Test-Enhanced Learning" (2006), `https://doi.org/10.1111/j.1467-9280.2006.01693.x`; Karpicke and Blunt (2011), `https://doi.org/10.1126/science.1199327` | Retrieval practice produces durable learning and often beats rereading or passive concept mapping. | Make recall attempts the default review surface and keep KB/source text hidden until after an attempt when practicing memory. | `src/tutor-engine.js`, `public/app.js`, `tests/tutor-engine.test.js` | Interleaved due-review queue and delayed retention checks. |
| Dunlosky et al., "Improving Students' Learning With Effective Learning Techniques" (2013), `https://doi.org/10.1177/1529100612453266` | Practice testing and distributed practice have stronger evidence than highlighting, rereading, or summarization alone. | Put active recall and spaced review above passive notes in the learning loop. | `src/tutor-engine.js`, `src/smart-resume.js`, `public/app.js` | Replace lightweight scheduler with stronger spacing policy. |
| Chi et al., self-explanation line (1989/1994), `https://doi.org/10.1207/s15516709cog1302_1`, `https://doi.org/10.1207/s15516709cog1803_3`; Chi and Wylie ICAP (2014), `https://doi.org/10.1080/00461520.2014.965823` | Learners build deeper understanding when they generate explanations, inferences, and repairs. | Teach-back, student mode, duck mode, and misconception repair are core features, not optional UX. | `src/tutor-engine.js`, `public/app.js`, `tests/tutor-engine.test.js` | Claim-level why/how follow-up routing. |
| Butterfield and Metcalfe, hypercorrection (2001), `https://doi.org/10.1037/0278-7393.27.6.1491`; Metcalfe, "Learning from Errors" (2017), `https://doi.org/10.1146/annurev-psych-010416-044022` | Mistakes, especially confident mistakes, are useful if the learner corrects them instead of passively receiving the answer. | Treat error repair and implicit contradiction detection as first-class tutor moves. | `src/tutor-engine.js`, `tests/tutor-engine.test.js` | Extend self-correction beyond comparison reversals to broader semantic contradictions. |
| Gick and Holyoak analogical transfer (1980/1983), `https://doi.org/10.1016/0010-0285(80)90013-4`, `https://doi.org/10.1016/0010-0285(83)90002-6`; Gentner structure mapping (1983), `https://doi.org/10.1207/s15516709cog0702_3` | Real transfer depends on mapping relational structure, not matching surface details. | Transfer prompts must ask what stayed invariant and what changed. | `src/tutor-engine.js`, `src/lesson-script.js`, `tests/tutor-engine.test.js` | Generate controlled near-transfer and far-transfer tasks. |
| Flavell metacognition (1979), `https://doi.org/10.1037/0003-066X.34.10.906`; Koriat monitoring model (1997), `https://doi.org/10.1037/0033-295X.104.2.349`; Butler and Winne feedback/SRL (1995), `https://doi.org/10.3102/00346543065003245` | Learners need to monitor knowledge, predict confidence, use feedback, and adjust strategy. | Store confidence prediction versus score and use the gap to guide smart resume. | `src/tutor-engine.js`, `src/learner-state.js`, `tests/tutor-engine.test.js` | Add UI confidence input before every teach-back and quiz score. |
| Sweller cognitive load (1988), `https://doi.org/10.1016/0364-0213(88)90023-7`; Wood, Bruner, and Ross scaffolding (1976), `https://doi.org/10.1111/j.1469-7610.1976.tb00381.x`; Kapur productive failure (2008), `https://doi.org/10.1080/07370000802212669` | Instruction should manage working-memory load, support learners in the productive zone, and use attempt-before-instruction when appropriate. | Keep tutor moves small: attempt, hint, worked example, teach-back, repair, transfer. | `src/tutor-engine.js`, `src/lesson-script.js`, `docs/tutor-system-architecture.md` | Explicit productive-attempt gate for new/hard concepts. |
...
| Implicit contradiction and self-correction checks | Feynman integrity, conceptual change, hypercorrection, learning from errors | `src/tutor-engine.js`, `public/app.js`, `tests/tutor-engine.test.js` | Implemented for faster/slower comparison reversals; semantic contradiction detection remains TODO. |
