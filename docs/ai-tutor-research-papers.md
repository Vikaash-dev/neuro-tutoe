...
| Delikoura et al., "From Superficial Outputs to Superficial Learning" (2025), `https://arxiv.org/abs/2509.21972` | LLM education risks include over-reliance, shallow understanding, hallucination, and reduced learner agency. | Require teach-back, active recall, Socratic prompts, self-check scores, and source traces so the learner must do work. | `src/tutor-engine.js`, `src/tutor-eval.js`, `src/citation-notebook.js`, `public/app.js` | Add over-helping detector and "ask before answer" policy tests. |
